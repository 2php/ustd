...
pub fn amax(vf32 x) noexcept-> u32 {
    ...
    let cnt = x.count();
    let res = f(cnt, x._data, x._step[0]);
    return res;
}
...
pub fn axpy(f32 alpha, vf32 x, vf32 y) -> void {
    ...
    let cnt = y.count();
    f(cnt, alpha, x._data, x._step[0], y._data, y._step[0]);
}
...
pub fn copy(vf32 x, vf32 y) -> void {
    ...
    let cnt = x.count();
    f(cnt, x._data, x._step[0], y._data, y._step[0]);
}
...
pub fn dot(vf32 x, vf32 y) noexcept->f32 {
    ...
    let cnt = x.count();
    let res = f(cnt, x._data, x._step[0], y._data, y._step[0]);;
    return res;
}
...
pub fn rot(vf32 x, vf32 y, f32x4 h) -> void {
    ...
    if (bool(h[0]==h[3]) && bool(h[1]==-h[2])) {
        ...
        let c = +h[0];
        let s = -h[1];
        f1(cnt, x._data, x._step[0], y._data, y._step[0], c, s);
    }
    else {
        f2(cnt, x._data, x._step[0], y._data, y._step[0], h._arr);
    }
}
...
pub fn gemv(f32 alpha, mf32 A, vf32 x, f32 beta, vf32 y) -> void {
    ...
    f(Layout::Col, trans, A._dims[0], A._dims[1], alpha, A._data, A._step[1], x._data, x._step[0], beta, y._data, y._step[1]);
}
...
pub fn gemv(f64 alpha, mf64 ma, vf64 vx, f64 beta, vf64 vy) -> void {
    ...
    f(Layout::Col, get_trans(ma), ma._dims[0], ma._dims[1], alpha, ma._data, ma._step[1], vx._data, vx._step[0], beta, vy._data, vy._step[1]);
}
...
pub fn ger(f32 alpha, vf32 x, vf32 y, mf32 A) -> void {
    ...
    let trans = get_trans(A);
    f(Layout::Col, trans, A._dims[0], A._dims[1], alpha, x._data, x._step[0], y._data, y._step[0], A._data, A._step[1]);
}
...
pub fn ger(f64 alpha, vf64 vx, vf64 vy, mf64 A) -> void {
    ...
    let trans = get_trans(A);
    f(Layout::Col, trans, A._dims[0], A._dims[1], alpha, vx._data, vx._step[0], vy._data, vy._step[0], A._data, A._step[1]);
}
...
pub fn gemm(f32 alpha, mf32 A, mf32 B, f32 beta, mf32 C) -> void {
    ...
    f(Layout::Col, trans_a, trans_b, C._dims[0], C._dims[1], A._dims[1], alpha, A._data, A._step[1], B._data, B._step[1], beta, C._data, C._step[1]);
}
...
pub fn gemm(f64 alpha, mf64 A, mf64 B, f64 beta, mf64 C) -> void {
    ...
    f(Layout::Col, Trans::NoTrans, Trans::NoTrans, C._dims[0], C._dims[1], A._dims[1], alpha, A._data, A._step[1], B._data, B._step[1], beta, C._data, C._step[1]);
}
...
pub fn getrf(mf32 A, vu32 vp) -> void {
    ...
    f(Layout::Col, A._dims[0], A._dims[1], A._data, A._step[1], vp._data);
}
...
pub fn geqrf(mf32 A, vf32 tau) -> void {
    ...
    f(Layout::Col, A._dims[0], A._dims[1], A._data, A._step[1], tau._data);
}
...
pub fn gesvd(mf32 A, vf32 s, mf32 U, mf32 VT) -> void {
    ...
    let info  = f(Layout::Col, jobu, jobvt, A._dims[0], A._dims[1], A._data, A._step[1], s._data, U._data, U._step[1], VT._data, VT._step[1], nullptr);
    (void)info;
}
...
pub fn getrs(mf32 A, vu32 vp, vf32 b) -> void {
    ...
    f(Layout::Col, A._dims[0], A._data, A._step[1], vp._data, b._data, b._step[1]);
}
